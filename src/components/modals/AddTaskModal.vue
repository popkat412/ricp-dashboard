    ...
    >
      ...
      <div>
        ...
        <select
          ...
          v-model="scoreFnName"
        >
          <option v-for="i in SCORE_FN_NAMES" :value="i">{{ capsFirstLetter(i) }}</option>
        </select>
      </div>
...
const validateDate = (v: Ref<string> | string, name: string) => {
  ...
  if (isNaN(d.getTime())) {
    snackbar.add({ type: "error", title: `${name} is not a valid date in format YYYY-MM-DD` });
    return null;
  }
  return d;
}
...
const addMember = async () => {
  console.log({title: title.value, description: description.value, expiryDate: expiryDate.value, scoreFnName: scoreFnName.value, baseScoreFnParams: baseScoreFnParams.value, extraScoreFnParams: extraScoreFnParams.value});
