<template>
  <div
    class="grid gap-3 grid-cols-[repeat(auto-fit,_minmax(0,_20rem))]"
    :class="[$props.centreAlign ? 'justify-center' : '']"
  >
    <div
    ...
    >
      <div class="w-full">
        <div class="flex flex-row">
          ...
          <div class="flex flex-col">
            <div class="text-centre text-2xl font-bold">
              {{ getScore(task) }}
            </div>
            <div>points</div>
          </div>
        ...
        <div class="flex flex-row">
          ...
          <div class="relative">
            <button
              ...
              @click="
                $props.onCompleted!(task instanceof Task ? task : task.task)
              "
...
const getTitle = (task: Task | TaskCompleted) =>
  task instanceof Task ? task.title : task.task.title;
const getDescription = (task: Task | TaskCompleted) =>
  task instanceof Task ? task.description : task.task.description;
const getScoreFnName = (task: Task | TaskCompleted) =>
  task instanceof Task ? task.scoreFnName : task.task.scoreFnName;
const getScore = (task: Task | TaskCompleted) =>
  task instanceof Task
    ? task.getScore(new Date())
    : task.task.getScore(task.dateCompleted);
const getDateAdded = (task: Task | TaskCompleted) =>
  task instanceof Task
    ? task.dateAdded.toLocaleDateString()
    : task.task.dateAdded.toLocaleDateString();
const getExpiryDate = (task: Task | TaskCompleted) =>
  (task instanceof Task
    ? task.expiryDate
