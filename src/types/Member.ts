...
export class Member {
  ...
  static async fromId(id: string): Promise<Either<Member, Error>> {
    ...
    const data = (
      await getDoc<FirebaseMember>(
        doc(db, "members", id) as DocumentReference<FirebaseMember>
      )
    ).data();
    ...
    return [new Member(id, data.name, history), null];
  }
  ...
  get tasksCompleted(): TaskCompleted[] {
    return this.history.flatMap((v) =>
      v.task ? [{ task: v.task, dateCompleted: v.timestamp }] : []
    );
  }
